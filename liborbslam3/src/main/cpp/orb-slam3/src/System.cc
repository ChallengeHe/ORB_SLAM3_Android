...
//            mbActivateLocalizationMode(true), mbDeactivateLocalizationMode(false) {
        ...
        if (!fsSettings.isOpened()) {
            ...
            LOGE("Failed to open settings file at: %s", strVocFile.c_str());
            exit(-1);
        }
...
//        bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile);
        ...
        LOGI("Seq. Name: %s", strSequence.c_str());
        LOGI("Initialize the Tracking thread");
    ...
    cv::Mat
    System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp,
        ...
        for (list<cv::Mat>::iterator lit = mpTracker->mlRelativeFramePoses.begin(),
            ...
            if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO) {
                ...
                f << setprecision(6) << 1e9 * (*lT) << " " << setprecision(9) << twb.at<float>(0)
                  << " " << twb.at<float>(1) << " " << twb.at<float>(2) << " " << q[0] << " "
                  << q[1] << " " << q[2] << " " << q[3] << endl;
            } else {
                cv::Mat Tcw = (*lit) * Trw;
                cv::Mat Rwc = Tcw.rowRange(0, 3).colRange(0, 3).t();
    ...
    void System::SaveKeyFrameTrajectoryEuRoC(const string &filename) {
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO) {
                ...
                cv::Mat t = pKF->GetCameraCenter();
                f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9)
                  << t.at<float>(0) << " " << t.at<float>(1) << " " << t.at<float>(2) << " " << q[0]
                  << " " << q[1] << " " << q[2] << " " << q[3] << endl;
            }
        }
        f.close();
